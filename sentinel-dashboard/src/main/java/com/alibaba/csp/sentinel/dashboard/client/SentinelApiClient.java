...
public class SentinelApiClient {
    ...
    @Autowired
    private GatewayFlowRulesNacosPublisher gatewayFlowRulesNacosPublisher;

    @Autowired
    private GatewayFlowRulesNacosProvider gatewayFlowRulesNacosProvider;

    @Autowired
    private DegradeRulesNacosProvider degradeRulesNacosProvider;

    @Autowired
    private DegradeRulesNacosPublisher degradeRulesNacosPublisher;

    @Autowired
    private ParamFlowRulesNacosPublisher paramFlowRulesNacosPublisher;

    @Autowired
    private ParamFlowRulesNacosProvider paramFlowRulesNacosProvider;
     ...
     * @return
     */
    public List<FlowRuleEntity> fetchRulesFromNacosServer(String app, String ip, int port) {
        try {
            List<FlowRule> flowRules = flowRuleNacosProvider.getRules(app);
            return flowRules.stream().map(r -> FlowRuleEntity.fromFlowRule(app, ip, port, r)).collect(Collectors.toList());
        } catch (Exception e) {
            logger.warn("getting-rules happen errors from nacos server! ");
            return null;
        }
    }
     ...
     * @throws Exception
     */
    public void publishRuleToNacosServer(String app, List<FlowRuleEntity> rules) {
        try {
            List<FlowRule> flowRules = rules.stream().map(FlowRuleEntity::toRule).collect(Collectors.toList());
            flowRuleNacosPublisher.publish(app, flowRules);
        } catch (Exception e) {
            logger.warn("publish rules to nacos server encounter errors!");
        }
    }

    public List<GatewayFlowRuleEntity> fetchGatewayFlowRulesFromNacosServer(String app, String ip, int port) {
        try {
            List<GatewayFlowRule> gatewayFlowRules = gatewayFlowRulesNacosProvider.getRules(app);
            ...
            logger.warn("fetch gateway flow rules to encounter errors");
            return null;
        }
    }
     ...
     * @param rules
     */
    public void publishGatewayRulesToNacosServer(String app, List<GatewayFlowRuleEntity> rules) {
        try {
            List<GatewayFlowRule> gatewayFlowRules = rules.stream().map(GatewayFlowRuleEntity::toGatewayFlowRule).collect(Collectors.toList());
            gatewayFlowRulesNacosPublisher.publish(app, gatewayFlowRules);
        } catch (Exception e) {
            logger.warn("publish gatway rules to encounter errors");
        }
    }

    public List<DegradeRuleEntity> fetchDegradeRuleFromNacosServer(String app, String ip, int port) {
        try {
            List<DegradeRule> degradeRules = degradeRulesNacosProvider.getRules(app);
            return degradeRules.stream().map(rule -> DegradeRuleEntity.fromDegradeRule(app, ip, port, rule)).collect(Collectors.toList());
        } catch (Exception e) {
            logger.warn("fetch degrade rule from nacos server fail,app:{}", app);
            return null;
        }
    }

    public void publishDegradeRulesToNacosServer(String app, List<DegradeRuleEntity> rules) {
        try {
            List<DegradeRule> degradeRules = rules.stream().map(DegradeRuleEntity::toRule).collect(Collectors.toList());
            degradeRulesNacosPublisher.publish(app, degradeRules);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public List<ParamFlowRuleEntity> fetchParamFlowRulesFromNacosServer(String app, String ip, int port) {
        try {
            List<ParamFlowRule> paramFlowRules = paramFlowRulesNacosProvider.getRules(app);
            return paramFlowRules.stream().map(rule -> ParamFlowRuleEntity.fromAuthorityRule(app, ip, port, rule)).collect(Collectors.toList());
        } catch (Exception e) {
            logger.warn("fetch param flow rules from nacos server are fail,app:{}", app);
            return null;
        }
    }

    public void publishParamFlowRulesToNacosServer(String app, List<ParamFlowRuleEntity> rules) {
        try {
            List<ParamFlowRule> paramFlowRules = rules.stream().map(ParamFlowRuleEntity::toRule).collect(Collectors.toList());
            paramFlowRulesNacosPublisher.publish(app, paramFlowRules);
        } catch (Exception e) {
            logger.warn("publish param flow rules to nacos server fail,app:{}", app);
        }
    }
