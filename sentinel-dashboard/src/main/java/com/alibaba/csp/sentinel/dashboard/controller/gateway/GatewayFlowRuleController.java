...
//            List<GatewayFlowRuleEntity> rules = sentinelApiClient.fetchGatewayFlowRules(app, ip, port).get();
            Thread.sleep(200);
            List<GatewayFlowRuleEntity> rules = sentinelApiClient.fetchGatewayFlowRulesFromNacosServer(app,ip,port);
    ...
    public Result<GatewayFlowRuleEntity> addFlowRule(@RequestBody AddFlowRuleReqVo reqVo) {
        ...
        // 先从nacos处拉取一次流控规则
        List<GatewayFlowRuleEntity> gatewayFlowRuleEntities = sentinelApiClient.fetchGatewayFlowRulesFromNacosServer(app,ip,port);
        repository.saveAll(gatewayFlowRuleEntities);
...
//        }
        publishGatewayRulesToNacosServer(app, ip, port);
...
//        }
        publishGatewayRulesToNacosServer(app, entity.getIp(), entity.getPort());
...
//        }
        publishGatewayRulesToNacosServer(oldEntity.getApp(), oldEntity.getIp(), oldEntity.getPort());
    ...
    /**
     * 推送rules到nacos server注册中心
     *
     * @param app
     * @param ip
     * @param port
     */
    private void publishGatewayRulesToNacosServer(String app, String ip, Integer port) {
        List<GatewayFlowRuleEntity> rules = repository.findAllByMachine(MachineInfo.of(app, ip, port));
        sentinelApiClient.publishGatewayRulesToNacosServer(app, rules);
    }
}
